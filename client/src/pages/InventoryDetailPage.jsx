...
import { 
  ArrowLeft, TrendingDown, TrendingUp, AlertTriangle, Plus, Minus, 
  Settings, Target, Activity, Calendar, Truck, Sparkles 
} from 'lucide-react';
...
const generateHistoryData = (currentQty, range) => {
  ...
  for (let i = points - 1; i >= 0; i--) {
    const date = new Date();
    if (labelFormat === 'month') date.setMonth(date.getMonth() - i);
    ...
    data.push({ label: label, stock: qty });
  }
...
export default function InventoryDetailPage() {
  ...
  const fetchItem = () => {
    axios.get(`${API_URL}/api/inventory/item/${id}`) 
      .then(res => {
        const data = res.data;
        setItem(data);
        setNewThreshold(data.low_stock_threshold);
        setNewTarget(data.target_capacity || 100); 
        setHistoryData(generateHistoryData(data.quantity, '7D'));
        setLoading(false);
      })
      .catch(err => {
        console.error(err);
  ...
  // AI CALCULATION
  const daysRemaining = item.monthly_usage > 0 
    ? Math.floor(item.quantity / (item.monthly_usage / 30)) 
    : 99;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
            ...
            <p className="text-xs text-gray-400 text-right">Target: {item.target_capacity || 100}</p>
            ...
            <div className="mt-6 pt-6 border-t border-gray-100 dark:border-gray-700">
                ...
                <div className="flex gap-3">
                    <div className="flex items-center border border-gray-200 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700 overflow-hidden">
                        <button onClick={decrementRestock} className="p-3 px-4 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors"><Minus size={16} /></button>
          ...
          <div className="grid grid-cols-2 gap-4">
             ...
             <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 flex flex-col justify-between overflow-hidden relative">
                <div className="relative z-10">
                    ...
                    <div className="flex items-end gap-2">
                        <p className="text-2xl font-bold text-gray-900 dark:text-white">{item.monthly_usage || 0}</p>
             ...
             <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 relative group transition-all">
                <div className="flex items-center justify-between mb-2">
                    ...
                    {!isEditingSettings && (
                         <button onClick={() => setIsEditingSettings(true)} className="text-gray-400 hover:text-blue-600 opacity-0 group-hover:opacity-100 transition-opacity"><Settings size={14} /></button>
                    )}
                </div>
                {isEditingSettings ? (
                    <div className="space-y-3">
                        <div>
                            ...
                            <input type="number" className="w-full p-1 text-sm bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded dark:text-white" value={newThreshold} onChange={(e) => setNewThreshold(e.target.value)}/>
                        </div>
                        <div>
                            <label className="text-[10px] text-gray-400 block mb-1">Max (Target)</label>
                            <input type="number" className="w-full p-1 text-sm bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded dark:text-white" value={newTarget} onChange={(e) => setNewTarget(e.target.value)}/>
                        </div>
                    ...
                    <div className="space-y-3">
                        <div>
                             <div className="flex items-center gap-2 mb-1"><AlertTriangle size={14} className="text-orange-500" /><span className="text-[10px] text-gray-400 uppercase font-bold">Min Threshold</span></div>
                             <p className="text-lg font-bold text-gray-900 dark:text-white pl-6">{item.low_stock_threshold}</p>
                        </div>
                        <div>
                             <div className="flex items-center gap-2 mb-1"><Target size={14} className="text-blue-500" /><span className="text-[10px] text-gray-400 uppercase font-bold">Target Max</span></div>
          ...
          {/* AI INSIGHT CARD */}
          <div className="bg-linear-to-br from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 p-5 rounded-2xl border border-indigo-100 dark:border-indigo-800">
            <div className="flex items-start gap-4">
              <div className="p-3 bg-white dark:bg-gray-800 rounded-xl shadow-xs text-indigo-600">
                <Sparkles size={20} />
              </div>
              <div>
                <h4 className="font-bold text-indigo-900 dark:text-indigo-200 text-sm mb-1">AI Stock Projection</h4>
                <p className="text-indigo-700 dark:text-indigo-300 text-xs leading-relaxed">
                  Based on your monthly usage of <strong>{item.monthly_usage} units</strong>, this stock is estimated to last approximately <strong>{daysRemaining} days</strong>.
                </p>
              </div>
            </div>
          </div>

          {/* DETAILS CARD (Supplier & Restock) */}
          <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
             <h4 className="text-xs font-bold text-gray-400 uppercase mb-4 tracking-wider">Item Details</h4>
             <div className="space-y-4">
                <div className="flex justify-between items-center text-sm">
                    <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400">
                        <Truck size={16} /> Supplier
                    </div>
                    <span className="font-medium text-gray-900 dark:text-white">MedSupply Co.</span>
                </div>
                <div className="w-full h-px bg-gray-100 dark:bg-gray-700"></div>
                <div className="flex justify-between items-center text-sm">
                    <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400">
                        <Calendar size={16} /> Last Restock
                    </div>
                    <span className="font-medium text-gray-900 dark:text-white">{item.last_restock || 'Unknown'}</span>
                </div>
             </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 h-96 flex flex-col">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={historyData}>
                <defs><linearGradient id="colorStock" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#3b82f6" stopOpacity={0.3}/><stop offset="95%" stopColor="#3b82f6" stopOpacity={0}/></linearGradient></defs>
