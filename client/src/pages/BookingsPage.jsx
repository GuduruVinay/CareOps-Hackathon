...
export default function BookingsPage() {
  ...
  const proceedWithCancel = async () => {
    ...
    toast.promise(promise, {
        ...
        success: () => {
            fetchBookings(); 
            return 'Booking cancelled & email sent.';
        },
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-sm w-full p-6 border border-gray-200 dark:border-gray-700 transform transition-all scale-100">
                <div className="flex flex-col items-center text-center">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mb-6 text-sm">
                        Are you sure you want to cancel the <strong>{confirmModal.booking.service_type}</strong> for <strong>{confirmModal.booking.name}</strong>? This action cannot be undone.
                    </p>
                    <div className="flex gap-3 w-full">
                        <button 
                            onClick={() => setConfirmModal({ isOpen: false, booking: null })}
                        ...
                        <button 
                            ...
                            className="flex-1 px-4 py-2.5 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700 transition-colors shadow-lg shadow-red-200 dark:shadow-none"
                        >
                            Yes, Cancel
                        </button>
                    </div>
                        ...
                        <tr key={booking.id} className="hover:bg-blue-50/50 dark:hover:bg-gray-700/30 transition-colors group cursor-default">
                            ...
                            <td className="p-5 whitespace-nowrap">
                                <div className="flex justify-center gap-2"> 
                                    ...
                                    {/* Action Logic for Desktop */}
                                    {booking.status !== 'CANCELLED' ? (
                                        <>
                                            <button onClick={() => handleRemind(booking)} className="p-2 text-gray-500 hover:text-blue-600 bg-gray-100 dark:bg-gray-800 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors border border-transparent hover:border-blue-100 dark:hover:border-blue-900" title="Send Reminder"><Bell size={18} /></button>
                                            <button onClick={() => handleReschedule(booking)} className="p-2 text-gray-500 hover:text-amber-600 bg-gray-100 dark:bg-gray-800 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded-lg transition-colors border border-transparent hover:border-amber-100 dark:hover:border-amber-900" title="Reschedule"><CalendarClock size={18} /></button>
                                            <button onClick={() => initiateCancel(booking)} className="p-2 text-gray-500 hover:text-red-600 bg-gray-100 dark:bg-gray-800 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors border border-transparent hover:border-red-100 dark:hover:border-red-900" title="Cancel Booking"><Trash2 size={18} /></button>
                                        </>
                                    ) : (
                                        <button onClick={() => handleReschedule(booking)} className="p-2 text-amber-600 bg-amber-50 dark:bg-amber-900/20 hover:bg-amber-100 dark:hover:bg-amber-900/40 rounded-lg transition-colors border border-amber-100 dark:border-amber-900" title="Reschedule Cancelled Booking"><CalendarClock size={18} /></button>
                                    )}
                                </div>
                            </td>
                        </tr>
                        ...
                        <div className="grid grid-cols-4 gap-2 pt-2">
                            ...
                            {booking.status !== 'CANCELLED' ? (
                                <>
                                    ...
                                    <button onClick={() => handleReschedule(booking)} className="flex items-center justify-center gap-2 p-2 rounded-lg bg-gray-50 dark:bg-gray-700/50 text-gray-600 dark:text-gray-300 font-medium text-xs hover:bg-amber-50 dark:hover:bg-amber-900/20 hover:text-amber-600 transition-colors"><CalendarClock size={14} /></button>
                                ...
                                <button onClick={() => handleReschedule(booking)} className="col-span-3 flex items-center justify-center gap-2 p-2 rounded-lg bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 font-medium text-xs hover:bg-amber-100 dark:hover:bg-amber-900/40 transition-colors">
                                    <CalendarClock size={14} /> Reschedule this booking
                                </button>
                            )}
                        </div>
                    </div>
